...
export function spawnFFmpeg(audioBitrate: number, videoBitrate: number, frameRate: number,
  ...
  // Use system FFmpeg for VA-API hardware encoding when available (VP8→H264 transcode).
  ...
  LOG.info("FFmpeg: using %s (%s).", ffmpegPath, useVaapi ? "VP8→H264 via h264_vaapi" : "copy passthrough");
  ...
  if(useVaapi) {
    ...
    // VP8 input → H264 output via VA-API. VP8 decode is software (fast), H264 encode is GPU.
    // Force keyframes every 2s so frag_keyframe flushes regularly.
    ffmpegArgs.push(
    ...
    // No GPU: copy video through unchanged (only works if input is already H264).
